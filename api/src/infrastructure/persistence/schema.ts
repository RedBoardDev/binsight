...
// Copy-bot · DEAD TABLE (C2-03): this was the P2 paper shadow-log. The paper engine was removed, so NOTHING writes or
// reads `copy_decisions` in production today — do not treat it as a live decision log. Kept (no drop migration) only
// to avoid a schema change; dropping it is a separate data-model task (mind the migration-hygiene finding H1-01).
