...
describe('UserRuntime — per-user, per-token concurrency cap (ULTRACODE #9)', () => {
  ...
  it('the DEFAULT (maxConcurrentPerToken 1) blocks a SECOND same-mint open; a different token is still allowed', () => {
    // WHY: the close-path residual sell reads the WHOLE wallet balance of the mint, so two concurrent same-mint
    // positions would commingle — the first to close would sell (and account for) the other's token leg too. The
    // default cap of 1 REJECTS the second same-mint open before it exists, so each closing position's whole-balance
    // read is its OWN token leg.
...
describe('UserRuntime — a RESYNC records the tracked size from PUBLISHED ops only (finding #145)', () => {
  ...
  it('a DEFERRED grow does NOT clobber an intervening resync that recorded a SMALLER size (finding #163)', async () => {
    ...
    // over-counted up to ~4×, spurious open-blocking). The monotonic per-position record seq marks the
