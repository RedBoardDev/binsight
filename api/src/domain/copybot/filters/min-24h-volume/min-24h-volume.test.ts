...
describe('min24hVolumeUsd — skip thin tokens (per-leader, cached, Jupiter v2 stats24h)', () => {
  ...
  it('volume below threshold → skip below_min_24h_volume', () => {
    expect(min24hVolumeUsd.evaluate(on(1000), c, ctx(500))).toEqual({
      action: 'skip',
      reason: 'below_min_24h_volume',
    });
  });
  ...
  it('unknown volume → skip min_24h_volume_unavailable', () => {
    expect(min24hVolumeUsd.evaluate(on(1000), c, ctx())).toEqual({
      action: 'skip',
      reason: 'min_24h_volume_unavailable',
    });
  });
  it('meta: jupiter-token / cached / no numeric preset', () => {
    expect([
      min24hVolumeUsd.source,
      min24hVolumeUsd.speedClass,
      min24hVolumeUsd.safePreset,
