...
const ctx = (tokenAgeHours?: number): FilterContext => ({
...
describe('minTokenAgeHours — skip young tokens (per-leader, cached, Jupiter v2 firstPool)', () => {
  ...
  it('age below threshold → skip below_min_token_age', () => {
    expect(minTokenAgeHours.evaluate(on(2), c, ctx(1))).toEqual({
      action: 'skip',
      reason: 'below_min_token_age',
    });
  });
  ...
  it('unknown age → skip min_token_age_unavailable (no open on unverifiable data)', () => {
    expect(minTokenAgeHours.evaluate(on(2), c, ctx())).toEqual({
      action: 'skip',
      reason: 'min_token_age_unavailable',
    });
  });
  it('meta: jupiter-token / cached / preset 2', () => {
    expect([
      minTokenAgeHours.source,
      minTokenAgeHours.speedClass,
      minTokenAgeHours.safePreset,
    ]).toEqual(['jupiter-token', 'cached', 2]);
  });
});
