...
const ctx = (priceRangePercent?: number): FilterContext => ({
...
describe('minPriceRangePercent — reject ultra-tight ranges (per-leader, instant, leader-shape)', () => {
  ...
  it('range below threshold → skip below_min_price_range', () => {
    expect(minPriceRangePercent.evaluate(on(3), c, ctx(1))).toEqual({
      action: 'skip',
      reason: 'below_min_price_range',
    });
  });
  ...
  it('unknown range → skip min_price_range_unavailable', () => {
    expect(minPriceRangePercent.evaluate(on(3), c, ctx())).toEqual({
      action: 'skip',
      reason: 'min_price_range_unavailable',
    });
  });
  it('meta: leader-shape / instant / preset 3', () => {
    expect([
      minPriceRangePercent.source,
      minPriceRangePercent.speedClass,
      minPriceRangePercent.safePreset,
    ]).toEqual(['leader-shape', 'instant', 3]);
  });
});
