...
describe('minMarketCapUsd — skip low-cap tokens (per-leader, cached, Jupiter v2 mcap)', () => {
  ...
  it('mcap below threshold → skip below_min_market_cap', () => {
    expect(minMarketCapUsd.evaluate(on(1_000_000), c, ctx(500_000))).toEqual({
      action: 'skip',
  ...
  it('unknown mcap → skip min_market_cap_unavailable', () => {
    expect(minMarketCapUsd.evaluate(on(1_000_000), c, ctx())).toEqual({
      action: 'skip',
      reason: 'min_market_cap_unavailable',
    });
  });
  it('meta: jupiter-token / cached / preset $1M', () => {
    expect([
      minMarketCapUsd.source,
      minMarketCapUsd.speedClass,
      minMarketCapUsd.safePreset,
    ]).toEqual(['jupiter-token', 'cached', 1_000_000]);
  });
});
