...
describe('minJupOrganicScore — primary anti-rug signal (per-leader, external-call, Jupiter v2)', () => {
  ...
  it('score below threshold → skip below_min_organic_score', () => {
    expect(minJupOrganicScore.evaluate(on(50), c, ctx(40))).toEqual({
      action: 'skip',
      reason: 'below_min_organic_score',
    });
  });
  ...
  it('unknown score → skip min_organic_score_unavailable (no open without the score)', () => {
    expect(minJupOrganicScore.evaluate(on(50), c, ctx())).toEqual({
      action: 'skip',
  ...
  it('meta: jupiter-token / external-call / preset 50', () => {
    expect([
      minJupOrganicScore.source,
      minJupOrganicScore.speedClass,
      minJupOrganicScore.safePreset,
    ]).toEqual(['jupiter-token', 'external-call', 50]);
  });
});
