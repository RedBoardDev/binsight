...
describe('render-user · pairLabel (SPEC §3.2)', () => {
  ...
  it('falls back to a 4-char mint truncation when the symbol is unknown', () => {
    ...
    const e = event('lifecycle.open_confirmed', {
      adminDetail: { mint: 'EKpQGSJtjMFqKZ9KQanSqYXRcF8fBopzLHYxdM65zcjm' },
    });
...
describe('render-user · internal codes never reach the feed (SPEC §3.2 + §6 loop guard)', () => {
  it('returns null for an internal code', () => {
    ...
    expect(
      toUserMessage(event('detect.observed', { audience: 'internal', category: 'DETECT' })),
    ).toBeNull();
    expect(
      toUserMessage(event('sign.land_failed', { audience: 'internal', category: 'SIGN' })),
    ).toBeNull();
  });
});
...
describe('render-user · SOL-only feed templates (SPEC §3.2)', () => {
  ...
  it('open', () => {
    const m = toUserMessage(
      event('lifecycle.open_confirmed', {
        leaderSizeSol: 12,
        ourSizeSol: 0.85,
        adminDetail: { nonSolSymbol: 'WIF', openCount: 3 },
      }),
    );
  ...
  it('close (intended size, D-6 labelled)', () => {
    const m = toUserMessage(
      event('lifecycle.close_confirmed', {
        stage: 'close',
        ourSizeSol: 0.85,
        signature: 'aaaaaaaaaaaa11111111zzzzzzzzzzzz',
        adminDetail: { nonSolSymbol: 'WIF' },
      }),
    );
    expect(m).toMatchSnapshot();
  ...
  it('partial-remove', () => {
    const m = toUserMessage(
      event('lifecycle.remove_partial', {
        stage: 'reshape',
        adminDetail: { nonSolSymbol: 'WIF', removedPct: 35 },
      }),
    );
    expect(m).toMatchSnapshot();
  });
  ...
  it('add (intended size, first add, D-6 labelled)', () => {
    const m = toUserMessage(
      event('lifecycle.add_confirmed', {
        stage: 'reshape',
        ourSizeSol: 0.4,
        adminDetail: { nonSolSymbol: 'WIF', firstAdd: true },
      }),
    );
    expect(m).toMatchSnapshot();
  });
  ...
  it('add — infinite-add off variant', () => {
    const m = toUserMessage(
      event('cap.infinite_add_skipped', {
        category: 'CAP',
        stage: 'reshape',
        outcome: 'blocked',
        adminDetail: { nonSolSymbol: 'WIF' },
  ...
  it('swap (intended SOL out, D-6 labelled)', () => {
    const m = toUserMessage(
      event('swap.executed', {
        ...
        stage: 'sell',
        ourSizeSol: 0.31,
        signature: 'bbbbbbbbbbbb22222222yyyyyyyyyyyy',
        adminDetail: { nonSolSymbol: 'WIF', swapInAmount: 124_500 },
      }),
    );
    expect(m).toMatchSnapshot();
  });
  ...
  it('insufficient-balance (CONFIGURED value, D-2 labelled)', () => {
    const m = toUserMessage(
      event('balance.insufficient', {
        ...
        stage: 'open',
        outcome: 'skipped',
        adminDetail: { nonSolSymbol: 'WIF', configuredSol: 0.42, requiredSol: 0.85 },
      }),
    );
    expect(m).toMatchSnapshot();
  });
  ...
  it('skipped-filter (transparency)', () => {
    const m = toUserMessage(
      event('filter.below_min_market_cap', {
        category: 'FILTER',
        stage: 'open',
        outcome: 'skipped',
  ...
  it('failsafe-activated', () => {
    const m = toUserMessage(
      event('failsafe.activated', {
        ...
        signature: 'cccccccccccc33333333xxxxxxxxxxxx',
        adminDetail: { nonSolSymbol: 'WIF' },
      }),
    );
    expect(m).toMatchSnapshot();
  ...
  it('failsafe-failed (close manually link)', () => {
    const m = toUserMessage(
      event('failsafe.failed', {
        ...
        outcome: 'failed',
        reason: 'failed',
        adminDetail: { nonSolSymbol: 'WIF', meteoraUrl: 'https://app.meteora.ag/dlmm/POOL' },
      }),
    );
    expect(m).toMatchSnapshot();
  });
  ...
  it('swap-failed (jup manual link)', () => {
    const m = toUserMessage(
      event('swap.failed_after_retries', {
        category: 'SWAP',
        severity: 'error',
        pinned: true,
        stage: 'sell',
