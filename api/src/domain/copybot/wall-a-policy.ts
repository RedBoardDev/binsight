 ...
 *    canonical Jito tip accounts — and only up to a per-transfer lamport cap. The System
...
export interface WallAPolicyInput {
  ...
  /** Hard ceiling on a single System.Transfer's lamports (defense in depth against an inflated wrap/tip). */
 ...
 * 8 Jito tips} — no foreign address can appear. Callers must pass their real ATAs; anything omitted simply can't
 * receive SOL (fail-closed).
 */
