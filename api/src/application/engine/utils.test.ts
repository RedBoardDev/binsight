...
// Mirrors the production schedule (engine/wallet-actor.ts REALIZED_REFRESH_OFFSETS_MS) so the WHY is
// encoded against real values: front-loaded offsets after a close — Helius indexes parsed txs in ~1-2s,
// so the bottleneck is the close→sell delay, not indexing.
...
describe('shouldRefreshRealized — front-loaded post-close realized-PnL refresh', () => {
  ...
  it('stops after the last offset (bounded cost — no indefinite re-fetch)', () => {
    const close = 1_000_000;
    expect(
      shouldRefreshRealized({
  ...
  it('catches up multiple missed checkpoints in one fire (the actor was busy past several offsets)', () => {
    // If the wallet's queue was busy, several offsets may have elapsed since the last pass — one fire is
