...
import { COPIER_TEST, connection, POOL_STABLE, solBalance } from './env';
 ...
 */
async function expectResyncRouted(
  position: string,
  timeoutMs = RESYNC_DECISION_TIMEOUT_MS,
): Promise<void> {
  ...
  throw new Error(
    `timeout: brain never routed a resync for leader ${position} within ${timeoutMs}ms — the remove event was MISSED`,
  );
}

describe.runIf(process.env.ONCHAIN_READY === 'true')(
  'on-chain · sweep reserve + edge actions (FEATURE 7.4 + 9.1/9.2/9.4)',
  () => {
    let h: Harness;
    beforeAll(async () => {
    ...
    const pollCopierPositions = async (
      pool: string,
      predicate: (p: UserPosition[]) => boolean,
      timeoutMs = POSITIONS_SETTLE_MS,
    ...
    it('7.4 sweep keeps the SOL reserve — two-sided close → token swept, native SOL not drained', async () => {
      ...
      expect(
        clean,
        'copier still holds a non-SOL token after the sweep window — DORMANT TOKEN',
      ).toBe(true);
      ...
      expect(
        solAfter,
        `copier SOL drained to ${solAfter.toFixed(4)} — the always-to-SOL sweep must keep the gas reserve, never sweep SOL itself`,
      ).toBeGreaterThanOrEqual(MIN_SOL_RESERVE);
      console.log(
        `💰 7.4 copier SOL: mid-open ${solMidOpen.toFixed(4)} → after sweep ${solAfter.toFixed(4)} (≥ reserve ${MIN_SOL_RESERVE})`,
      );
    }, 240_000);
    ...
    it('9.2 remove-to-zero (not close) — emptied leader kept open; close then resets clean, no dormant', async () => {
      ...
      if (await h.accountExists(leaderPos)) {
        ...
        expect(
          await h.accountExists(copy),
          'copy closed while the leader is still open-but-empty (documented: emptied→still-open)',
        ).toBe(true);
        console.log(
          '📍 9.2 leader emptied-but-OPEN → bot saw the remove (resync routed), copy kept open',
        );
      } else {
        // The bench's remove(100%) auto-closed the leader (not the targeted empty-but-open path) — the close path cleans up.
        console.warn(
          '9.2: leaderRemove(100%) auto-closed the leader position — the close/reconcile path will clean the copy',
        );
      }
      ...
      expect(
        await h.accountExists(copy),
        'copy still on-chain after close — DORMANT POSITION',
      ).toBe(false);
      const end = await pollCopierPositions(POOL_STABLE, (p) => p.length === 0);
      expect(
        end.length,
        'copier still tracks a position on the pool after close — DORMANT POSITION',
      ).toBe(0);
    }, 240_000);
    ...
    it('9.4 reopen after close — the second copy is fresh and distinct, no stale state from the first', async () => {
      ...
      expect(
        copy2,
        'reopen produced the SAME copy pubkey as the closed one — stale resurrection',
      ).not.toBe(copy1);
      expect(await h.accountExists(copy2), 'second copy did not land on-chain').toBe(true);
      expect(
        await h.accountExists(copy1),
        'the first (closed) copy reappeared on-chain — stale resurrection',
      ).toBe(false);
    }, 240_000);
  },
);
