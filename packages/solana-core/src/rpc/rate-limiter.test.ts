...
describe('SolanaRpcRateLimiter', () => {
  ...
  it('applies the same discovery sub-limit to both owner/program V2 methods', () => {
    const lim = new SolanaRpcRateLimiter(limits, () => 1000);
    expect(lim.reserveSlot('getProgramAccountsV2')).toBe(1000);
    expect(lim.reserveSlot('getTokenAccountsByOwnerV2')).toBe(1200);
    expect(lim.stats().gpa).toBe(2);
  });
